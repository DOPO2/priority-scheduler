...
main ()
{
  ...
  float total_waitingTime = 0, total_turnaroundTime =
    0, Avg_waitingTime, Avg_turnaroundTime;
  ...
  for (t = wake_time; t < 24; t++)
      ...
      if (t == d_time)
	{
      ...
      for (int i = 0; i < n; i++)
	{
	  if (a[highest_p].priority < a[i].priority && a[i].arrivalTime <= t
	      && a[i].burstTime > 0)
	    {
	      highest_p = i;
	...
	{
	  count++;
	  a[highest_p].waitingTime =
	    t + 1 - a[highest_p].arrivalTime - temp[highest_p];
	  ...
	  total_turnaroundTime =
	    total_turnaroundTime + a[highest_p].turnaroundTime;
	}
    }
  ...
  for (int i = 0; i < n; i++)
    {
      printf ("%d\t\t%d\t\t%d\n", i, a[i].waitingTime, a[i].turnaroundTime);
    }
  printf ("\n");
